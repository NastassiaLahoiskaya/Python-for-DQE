...
def create_list_of_dict(random_number_from=1, random_number_to=2, random_letter_from=10, random_letter_to=1):
    ...
    for x in range(random.randint(random_number_from, random_number_to)):  # generate random numbers of dictionaries
        ...
        values = (random.randint(random_letter_to, random_letter_from) for y in
                  range(size))  # random numbers in value parameters
...
dict_list = create_list_of_dict(random_number_from=2, random_number_to=10,
                                random_letter_from=50)  # call the function with parameters
...
distinct_keys, duplicated_keys = create_lists_with_keys(
    dict_list)  # call the function create_list_of_dict with parameter dictList
    ...
    for key in distinct_keys:  # check if the letter is exists in 'duplicated_keys' list
        if key in duplicated_keys:
            ...
            for i, rand_dict in enumerate(
                    dict_list):  # iterate over each dictionary and check if the specified key is in
